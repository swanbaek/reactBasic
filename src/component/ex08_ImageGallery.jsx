...
export default function ImageGallery() {
    ...
    /**filter() 함수는 원래 배열의 각 요소에 대해 함수를 호출하고, 그 함수가 true를 반환하는 
      요소만을 포함하는 새로운 배열을 반환합니다.  이 함수는 조건에 맞는 요소들만을 필터링하여 새로운 배열을 생성
      filter() 함수는 원본 배열을 변경하지 않고 새로운 배열을 반환합니다. 
      React의 상태 관리 원칙에 따라 상태를 직접 변경하지 않고 새로운 상태를 설정하는 것. 
     */
    const deleteImage = (i) => {
        const tmpImgs = imgs.filter((image, idx) => {
            return i !== idx;
        });
        setImages(tmpImgs);
    };
            ...
            <Container className="py-5">
                <Row>
                    <Col md={4} className="mb-5">
                        <Button variant="warning" className="mx-1" onClick={viewImage}>
                            All Images {text}
                        </Button>
                        <Button variant="success" onClick={addImage}>
                            Add Image
                        </Button>
                    </Col>
                </Row>
